Title: Saved Routes — One-page Design Brief

Purpose:
- Browse, preview, and manage saved routes quickly.

Audience:
- UX, Product, PM.

Goals:
- Fast route selection for session creation; easy management (rename, favorite, delete).

Layout & Structure:
- List or grid with map thumbnails, filter/search bar, bulk actions.

Key Components:
- Route card (thumbnail, title, meta), favorite toggle, contextual menu.

States & Interactions:
- Quick preview on hover/press, multi-select for bulk delete, sort options.

Content Guidelines:
- Show distance/time/elevation in a single glance.

Accessibility:
- Large tap targets on cards, alt text for thumbnails.

Assets for Figma:
- Thumbnail component, list/grid layout tokens, placeholders.

Acceptance Criteria:
- Routes discoverable; user can start a session from a route in one tap.
