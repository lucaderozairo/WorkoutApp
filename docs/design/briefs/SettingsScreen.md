Title: Settings — One-page Design Brief

Purpose:
- Manage account, sync, privacy, and app preferences.

Audience:
- Users, PM, Support.

Goals:
- Make critical toggles discoverable and explainable; avoid overwhelming the user.

Layout & Structure:
- Grouped sections (Account, Integrations, Notifications, Privacy), search at top.

Key Components:
- Toggle rows, select rows, destructive actions area, change-password flow.

States & Interactions:
- Confirmation modals for destructive actions, inline help for integrations.

Content Guidelines:
- Short labels, optional explanatory text, link to help docs.

Accessibility:
- Large touch targets, clear focus states, screen-reader descriptions for toggles.

Assets for Figma:
- Settings row components, confirm modal patterns.

Acceptance Criteria:
- Users can find core toggles within 3 taps and understand effects.
