...
import { MOCK_SLEEP_WEEK } from '@data/mock/sleep';
...
interface WidgetInstance { id: string; size: WidgetSize; }
interface WidgetDef {
  id: string;
  label: string;
  sizes: WidgetSize[];
...
const lastNight = MOCK_SLEEP_WEEK[6];

const REGISTRY: WidgetDef[] = [
  { id: 'sleep-review', label: 'Sleep Review', sizes: ['1x1', '2x1', '2x2'], Component: ({ size }) => <SleepReviewWidget size={size} session={lastNight} /> },
...
function AddWidgetPanel({ activeIds, onAdd }: { activeIds: Set<string>; onAdd: (id: string) => void }) {
  ...
  return (
    <div className="surface compact column">
